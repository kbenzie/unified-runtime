...
UR_APIEXPORT ur_result_t UR_APICALL urDeviceGetInfo(ur_device_handle_t hDevice,
  ...
  case UR_DEVICE_INFO_TYPE:
    return ur::setInfo<ur_device_type_t>(UR_DEVICE_TYPE_GPU, propSize,
  ...
  case UR_DEVICE_INFO_PLATFORM:
    return ur::setInfo(hDevice->platform, propSize, pPropValue, pPropSizeRet);

  case UR_DEVICE_INFO_REFERENCE_COUNT:
    return ur::setInfo(hDevice->getRefCount(), propSize, pPropValue,
                       pPropSizeRet);
  ...
  case UR_DEVICE_INFO_NAME: {
    std::string name = hDevice->mDevice->name()->cString(
        NS::StringEncoding::ASCIIStringEncoding);
    return ur::setInfo(name, propSize, pPropValue, pPropSizeRet);
  }

  case UR_DEVICE_INFO_VENDOR: {
    std::string vendor = "Apple";
    return ur::setInfo(vendor, propSize, pPropValue, pPropSizeRet);
  }
  ...
  case UR_DEVICE_INFO_DRIVER_VERSION: {
    ...
    // auto driverVersion = std::to_string(hDevice->mDevice->registryID());
    // std::string driverVersion = "unknown";
    return ur::setInfo(driverVersion, propSize, pPropValue, pPropSizeRet);
  }
  ...
  case UR_DEVICE_INFO_VERSION: {
    std::string version = "";
    if (hDevice->mDevice->supportsFamily(MTL::GPUFamilyMetal3)) {
      version = "Metal3";
    } else if (hDevice->mDevice->supportsFamily(MTL::GPUFamilyCommon3)) {
      version = "Common 3";
    } else if (hDevice->mDevice->supportsFamily(MTL::GPUFamilyCommon2)) {
      version = "Common 2";
    } else if (hDevice->mDevice->supportsFamily(MTL::GPUFamilyCommon1)) {
      version = "Common 1";
    } else {
      throw UR_RESULT_ERROR_ADAPTER_SPECIFIC;
    }
    return ur::setInfo(version, propSize, pPropValue, pPropSizeRet);
  }
